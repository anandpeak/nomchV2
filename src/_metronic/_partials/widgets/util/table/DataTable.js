...
class DataTable extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            locale: props.locale,
            filter_value: "",
            page_size: props.config.show_all && props.records.length > 0 ? props.records.length : (props.config.page_size) ? props.config.page_size : 10,
            page_number: 1,
            selectedRow: null,
            sort: (props.config && props.config.sort) ? props.config.sort : {
        ...
        this.config = {
            button: {
                filterColumns: (props.config && props.config.button && props.config.button.filterColumns) ? props.config.button.filterColumns : false,
            ...
            language: {
                length_menu: (props.language && props.language.length_menu) ? props.language.length_menu : "_MENU_",
                filter: (props.language && props.language.filter) ? props.language.filter : translations(props.locale).search || "",
                info: (props.language && props.language.info) ? props.language.info : "Нийт _TOTAL_ илэрцийн _START_-с _END_-г харуулж байна",
                pagination: {
                    first: (props.language && props.language.pagination && props.language.pagination.first) ? props.language.pagination.first : '<i className="la la-angle-left"></i>',
                    previous: (props.language && props.language.pagination && props.language.pagination.previous) ? props.language.pagination.previous : '<i className="la la-angle-left"></i>',
                    next: (props.language && props.language.pagination && props.language.pagination.next) ? props.language.pagination.next : '<i className="la la-angle-right"></i>',
                    last: (props.language && props.language.pagination && props.language.pagination.last) ? props.language.pagination.last : '<i className="la la-angle-right"></i>'
                }
            },
            length_menu: (props.config && props.config.length_menu) ? props.config.length_menu : [10, 25, 50, 75, 100],
            no_data_text: (props.config && props.config.no_data_text) ? props.config.no_data_text : translations(props.locale).empty || "",
            ...
            show_all: (props.config.show_all != undefined) ? props.config.show_all : false,
            show_footer: (props.config.show_footer != undefined) ? props.config.show_footer : false,
    ...
    render() {
        ...
        return (
            <div className="as-react-table react-datatable" id={(this.props.id) ? this.props.id + "-container" : ""}>
                    ...
                    <div className="col-md-5 pl-0 pr-0">
                        {(this.props.config.add_button_left)
                            ? (
                                this.props.config.text_or_button === 'button'
                                    ?
                                    <button onClick={(e) => this.props.buttonLeftClick()}
                        ...
                        {(this.config.show_length_menu && !this.config.show_all) ? (
                            <div className="input-group asrt-page-length">
                                ...
                                {this.config.language && (_.includes(this.config.language.length_menu, '_MENU_')) ? (
                                    <select className="form-control" style={style.table_size_dropdown}
                ...
                <div className="row table-body asrt-table-body" style={style.table_body}
                            ...
                            <table className={this.props.className} id={this.props.id}>
                                ...
                                <tr>
                                    {
                                        columnList.map((column, index) => {
                                            let classText = (column.sortable) ? "sortable " : "non-sortable",
                                            ...
                                            return (
                                                <th
                                                    ...
                                                    width={width}
                                                    style={columnStyle}
                                                    onClick={() => this.props.hasCustomSort ? this.props.customSort(column, sortOrder) : this.sortColumn(column, sortOrder)}>
                                                    {
                                                        column.checkAll && column.colType === 'checkbox'
                                                            ...
                                                            column.clickableTh
                                                                ?
                                                                <div onClick={() => this.props.theadClick(column.key)}
                                                                     className="underline"
                                ...
                                {(filterRecords.length) ? filterRecords.map((record, rowIndex) => {
                                    ...
                                    return (
                                        <tr key={rowIndex}
                                            data-index={rowIndex}
                                            className={that.state.selectedRow === rowIndex ? "selected" : ""}
                                            onClick={() => this._onSelectRow(rowIndex)}
                                        >
                                            {
                                                columnList.map((column, colIndex) => {
                                                    return that._renderTd(this.props.staticFirstRow ? defaultRowIndex : rowIndex,
                                                        colIndex, column, record, hasContextMenu, contextMenus);
                                                })
                                            }
                                        </tr>
                                    )
            ...
            <td key={'row_' + rowIndex + '_col_' + colIndex + '_key_' + column.key} className={classText}>
                {
                    hasContextMenu
                        ?
                        <ContextMenuTrigger
                            id={'cm_' + '_row_' + rowIndex + '_col_' + colIndex + '_key_' + column.key}
                            attributes={{test: 'zzz'}}>
                            <div className={"trigger-inner-container"}>
                                {
                        ...
                        >
                            {
                                contextMenus.map(function (obj, i) {
                                    return (
                                        <MenuItem key={i}
                                                  onClick={() => that.props.contextMenuItemClick(rowData.id, obj.key)}>
                                            <i className={"m-nav__link-icon " + obj.iconClassName}></i>
                                            <span
                                                className="m-nav__link-text">{obj.text}</span>
                                        </MenuItem>
                                    )
                                })
                            }
        ...
        switch (colType) {
            case 'image':
                return record[column.key]
                    ? <img src={record[column.key]} className={"circle " + column.className}/>
                    : <img src={'/images/image_placeholder.jpg'} className={"circle " + column.className}/>;
            case 'radio':
                    ...
                    <Form>
                        {
                            this.props.radioOptions.map(function (radioObj, i) {
                                let checkedStyle = {
                                    ...
                                    boxShadow: '0 5px 10px 2px rgba(' + hexToRgb(radioObj.color).r + ', ' + hexToRgb(radioObj.color).g + ', ' + hexToRgb(radioObj.color).b + ', ' + 0.19 + ')'
                                };
                                let uncheckedStyle = {
                                        ...
                                        style={{display: 'inline-block'}}>
                                        {
                                            column.hasTooltip
                                                ...
                                                <Radio
                                                    ...
                                                    style={record.radioTypeId === radioObj.id ? checkedStyle : uncheckedStyle}
                                                />
                                                :
                                                <Radio
                                                    label={radioObj.shortName}
            ...
            case 'input':
                return <Input
                    ...
                    onBlur={(e) => {
                        this.props.onInputFocusOut(e, rowIndex, record.id, this.inputRefs)
                    }}
                    onKeyPress={(event) => {
            ...
            default:
                if (column.clickableTd && record.clickable) {
                    return <span
                        ...
                        onClick={() => {
                            this.props.tdClick(column.key, record.id)
                        }}>{column.priceFormat ? priceFormat(record[column.key]) : record[column.key]}</span>
                } else {
                    return column.priceFormat ? priceFormat(record[column.key]) : (record[column.key] ? record[column.key] :
                        <span>&nbsp;</span>)
                }
        }
    }
    ...
    toggleColumn(el, e, column) {
        ...
        let selectedColumns = filterColumns.filter(columnObj => columnObj.key === column.key);
            ...
            for (let column of this.state.filterColumns) {
                if (!column.hide) {
                    if (column.cell && typeof column.cell === "function") {
                        ...
                        sTable += "<td>" + (this.props.staticFirstRow && colIndex === 1 ? rowIndex : record[column.key]) + "</td>";
                    } else {
                        sTable += "<td></td>";
                    }
                    colIndex++;
    ...
    updateConfigText() {
        this.config.filename = this.config.filename || translations(this.state.locale).list;
        ...
        this.config.language.info = translations(this.state.locale).action.datatable_info;
